    ...
    <nav className="bg-indigo-900 text-white shadow-md">
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            <Link href="/cart" className="relative cursor-pointer">
              <div className="indicator flex items-center">
                ...
                <span className="badge badge-sm bg-purple-400 text-indigo-950 font-bold absolute -top-2 -right-2 text-xs px-1">8</span>
              </div>
            </Link>
            ...
            <div className="relative">
              ...
              {/* Dropdown Menu */}
              {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-40 bg-blue-500 text-black rounded-lg shadow-md">
                  <Link href="/profile" className="block px-4 py-2 hover:bg-purple-400">Profile</Link>
                  {isLoggedIn ? (
                    <Link href="/" onClick={() => setIsLoggedIn(false)} className="block px-4 py-2 hover:bg-purple-400">Logout</Link>
                  ) : (
                    <>
                      <Link href="/login" className="block px-4 py-2 hover:bg-purple-400">Login</Link>
                      <Link href="/register" className="block px-4 py-2 hover:bg-purple-400">Register</Link>
                    </>
                  )}
                </div>
              )}
      ...
      <div
        className={`fixed inset-0 bg-indigo-900 transition-transform transform ${isMenuOpen ? "translate-x-0" : "-translate-x-full"
          } md:hidden flex flex-col items-center justify-center space-y-6 text-lg font-semibold`}
      >
        {/* Close Button */}
        <button
